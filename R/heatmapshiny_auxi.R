...
value2color <- function(x, n=10) {
  if (is.numeric(x)) {
    nl <- cut(x,n)
    set.seed(861018)
    cp <- colorRampPalette(RColorBrewer::brewer.pal(n = 7, name = "Blues"))(n)
    names(cp) <- levels(nl)
    cc <- cp[nl]
    ...
    if (length(nl) > 60) {
      set.seed(861018)
      cp <- randomcoloR::distinctColorPalette(k = 60)
      cp <- rep(cp, ceiling(length(nl)/60))[1:length(nl)]
    } else {
      set.seed(861018)
      cp <- randomcoloR::distinctColorPalette(k = length(nl))
    }
    names(cp) <- nl
    cp <- cp[sort(names(cp))]
    cc <- cp[x]
...
#' @param object an object returned by \link{\code{addHeatmapAnnotation}}
...
adist <- function(x, method="pearson") {
  ...
  if (method %in% arg.dist) 
    dd <- stats::dist(x, method = method) else
...
shinyPlotTooltipsUI <- function(id) {
  ...
  tagList(
    ...
    absolutePanel(fixed=TRUE, draggable = TRUE, htmlOutput(ns("hover_info"))
    )
  )
}
