...
list2csc <- function(l, dimnames) {
  ...
  names <- as.character(dimnames[[1]])
  if (length(dimnames) > 1) {
    cn <- as.character(dimnames[[2]])
  } else {
    cn <- as.character(unique(l$gsId))
  }
  args <- list(
    i = c(fmatch(as.character(l$featureId), names)),
    j = c(fmatch(as.character(l$gsId), cn)),
