...
stringD3Net <- function(ntwk, gsa, i, label = FALSE) {
  ...
  links <- data.frame(
    source = fastmatch::fmatch(ntwk$preferredName_A, nd$name)-1,
    target = fastmatch::fmatch(ntwk$preferredName_B, nd$name)-1,
...
stringNetwork <- function(genes, taxid = 9606, caller = "ExpressionSetViewer") {
  ...
  params <- paste(
    sapply(names(params), function(x) paste(x, params[[x]], sep = "=")),
    collapse = "&"
  )
...
stringGSA <- function(genes, taxid = 9606, background = NULL, backgroundStringId = FALSE, caller = "ExpressionSetViewer") {
  ...
  params$identifiers = paste(genes, collapse = "%0d") # your protein
