...
plotly_scatter <- function(
  ...
  if (i1) {
    ...
    df <- beeswarm::beeswarm(y ~ x, corral = "wrap", do.plot = FALSE)
    df$index <- fastmatch::fmatch(rownames(df), paste(x, names(y), sep = "."))
    ...
    df$x <- df0$y
    df$y <- df0$x
    df$index <- fastmatch::fmatch(rownames(df), paste(y, names(x), sep = "."))
  ...
  cc <- randomcoloR::distinctColorPalette(k = length(unique(df$color)))
