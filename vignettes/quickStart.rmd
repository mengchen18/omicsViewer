...
```{r eval=FALSE}
# install dependencies on github
devtools::install_github('omarwagih/rmotifx')
if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("ExpressionSetViewer")
BiocManager::install("ExpressionSetViewer")
```
...
* **PTM** This tab will only be shown when users give a column as the "sequence window" in the 
feature data. The column should contain the peptide sequence with the same length. The enriched motif 
maybe identified according to user selected features. 
...
In addition to opening the viewer inside R, ExpressionSetViewer together with the data prepared, can be deployed via several other ways. 
...
* **Portable R:** To share the ExpressionSetViewer and data with users do not use docker, you can install ExpressionSetViewer in the [r portable](https://sourceforge.net/projects/rportable/).
